  ...
  return (
    <Card className="border-0 shadow-lg bg-white">
      <CardHeader className="p-4 flex flex-row items-center justify-between">
        ...
        <Button className="border border-gray-300 rounded-full px-2 py-1 flex flex-row items-center gap-1">
      ...
      <CardContent className="p-4 flex flex-col gap-2">
        {accounts.map((acc) => (
          ...
          >
            <View
              className="w-10 h-10 rounded-full flex items-center justify-center bg-white mr-3"
            >
              {acc.icon}
            </View>
            <View style={{ flex: 1 }}>
              <Text>{acc.name}</Text>
              {acc.status ? (
                <Text style={{ fontSize: 12, color: "#64748b" }}>{acc.status}</Text>
              ) : null}
              {acc.balance ? (
                <Text className="text-xs" style={{ fontWeight: "bold", color: "#059669" }}>
            ...
            <Feather name="chevron-right" size={20} color="#64748b" style={{ marginLeft: 8 }} />
          </TouchableOpacity>
        ))}
        <View
          style={{
        ...
        >
          ...
          <Text style={{ fontWeight: "bold", fontSize: 16 }}>R${total.toFixed(2)}</Text>
        </View>
      </CardContent>
    </Card>
  );
